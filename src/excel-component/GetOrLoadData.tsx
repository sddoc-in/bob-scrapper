...
export default function GetOrLoadData() {
  ...
  async function loadMoreData() {
    ...
    let currentProduct = selectedProduct.product
    ...
    try {
      ...
      let currentProductList = allProducts.filter(
        (item: any) => item.product === currentProduct
      );
      if (currentProductList.length > 0) {
        newProductList = allProducts.map((item: any) => {
          if (item.product === currentProduct) {
            item.page = currentPage + 3;
          }
          return item;
        });
      }
  ...
  return (
    <div className="flex justify-center items-center flex-col my-4">
      <div
        onClick={() => setSelectOpen({ ...selectOpen, open: !selectOpen.open })}
      ...
      >
        <input
          ...
          onChange={channgeProduct}
        />

        {selectOpen.open ? (
          <IoIosArrowDropupCircle
            className={`text-[30px] text-black cursor-pointer ${themeObj}`}
          />
        ) : (
          <IoIosArrowDropdownCircle
            className={`text-[30px] text-black cursor-pointer ${themeObj}`}
